...
const ProficiencyModifier = {
    ...
    fromLevelAndRank: (level: number, rank: number, options: { addLevel?: boolean } = {}): ModifierPF2e => {
        ...
        const modifier =
            [
                0,
                game.settings.get("ordem-paranormal", "proficiencyTrainedModifier") ?? 2,
                game.settings.get("ordem-paranormal", "proficiencyExpertModifier") ?? 4,
                game.settings.get("ordem-paranormal", "proficiencyMasterModifier") ?? 6,
                game.settings.get("ordem-paranormal", "proficiencyLegendaryModifier") ?? 8,
